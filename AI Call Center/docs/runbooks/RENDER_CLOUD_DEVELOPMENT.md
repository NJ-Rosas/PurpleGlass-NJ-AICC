...
4. The standard G.711 μ-law encoder converts every valid 8 kHz sample. As soon as the current explicit 280 ms startup reserve is available, PurpleGlass sends fourteen ordered 20 ms/160-byte packets and begins the monotonic low/high-water controller; send-ahead is hard-bounded at 300 ms.
...
Task 16.1.2 responds to the controlled-call report of a cut/restarted first word and intermittent playback jitter. Because production audio was intentionally not logged, the exact audible prefix mechanism is not conclusively reproducible. Local byte-continuity tests rule out duplicated OpenAI deltas, PCM carry, resampler windows, μ-law regions, packet extraction, and first-packet sends. They do reproduce the first transport discontinuity: the former 100 ms cadence could exhaust its playback reserve during provider starvation and could send multiple overdue packets in a catch-up burst after a late continuation.

The corrected quality controls are explicit configuration: `OutboundPacketDuration=20 ms`, `OutboundStartupBufferDuration=100 ms`, and `MaximumPacingLateness=40 ms`. Five initial packets establish the same 100 ms reserve used before, so there is no designed first-media latency increase. Subsequent packets use absolute monotonic deadlines. A late wake sends one packet and rebases the next deadline rather than bunching overdue packets. A true underflow accumulates a fresh bounded 100 ms reserve before restarting; it never buffers the whole response. A response-level `playback_pacing_summary` reports packet count, packet duration, startup/max buffer, underflows, and average/max lateness. The activity carries the same bounded numeric fields. Do not add packet-level logs or audio values.

The `final_speech_to_endpoint` latency stage is now measured from the last speech-bearing inbound frame to endpoint detection. No VAD threshold changed. During the next controlled validation, compare that stage with STT, LLM, TTS-first-audio, and first-media timing; do not tune silence thresholds unless the new measurement is material and natural pauses remain one turn.

Do not claim the live cut/replay or jitter is fixed from local tests alone. After release validation, deploy both services from one exact Task 16.1.2 SHA and make only the separately authorized controlled Task 16.2 call. Confirm the greeting and each response start cleanly, listen for jitter, exercise one barge-in/clear, and verify `underflow_count`, pacing lateness, and first-media timing using safe internal IDs only.

The `8445bf14ad06da4c167f91c4f5218d4b92e5a7ea` Task 16.2 revalidation failed and must not be promoted. The tester reported frequent cutoffs. Six completed responses recorded 17 underflows; every completed response underflowed, and maximum reported lateness clustered around 79–83 ms. The services were restored to the validated `c47a53352d8cfc57b8de62d9cba4ef9e245820be` baseline before Task 16.1.3 development.

Task 16.1.3 distinguishes the local μ-law queue from media already sent to Twilio. The initial five 20 ms packets already established a real 100 ms remote reserve, but the previous rebase preserved whatever smaller reserve remained after a late send. It did not replenish the deficit. Its lateness measurement also ended after the WebSocket write, combining timer/scheduler delay with send duration, and it had no explicit rebuffer counter. The corrected sender measures scheduler wake before the write, measures write duration separately, and maintains `sent audio duration - elapsed playback duration`, bounded at zero, as the sole remote-reserve estimate.

The production constants remain 20 ms/160 bytes with a 100 ms target. Startup sends exactly the bounded reserve already available. Steady state sends one scheduled packet, then only enough whole 20 ms packets to repair a measured reserve deficit without exceeding 100 ms. If producer starvation actually exhausts the remote reserve, the response records one starvation/underflow episode, rebuilds the bounded threshold, emits one observable rebuffer, and resumes once. It does not alternate isolated 20 ms packets and gaps, buffer the full response, or synthesize silence. Final responses shorter than the threshold flush their complete real audio and then mark. Cancellation/clear discards local bytes and the old generation's reserve/rebuffer state before the next response.

For the next controlled validation, inspect the aggregate `playback_pacing_summary` fields: `SchedulerLateCount`, average/maximum scheduler lateness, `ProducerStarvationCount`, `RemoteBufferUnderflowCount`, `RebufferCount`, `TotalRebufferMs`, initial reserve, minimum/maximum remote reserve, and maximum send duration. A scheduler-late event with remote reserve remaining is not an underflow. An underflow without a matching bounded rebuffer on a response that still has audio is a failure. Continue to log numeric state only. Do not claim the cutoff defect is fixed until one authorized human call against the next reviewed exact SHA passes.

The `912526db3ab35e65df6edbf6476a490a06733456` Task 16.2 candidate also failed the human audio-quality gate and must not remain deployed. The call completed and barge-in worked, but beginning jitter and repeated reserve exhaustion remained. Ten completed responses recorded 2,087 packets, 426 scheduler-late events (approximately 12.456 ms weighted average and 161.257 ms maximum), approximately 76.115 ms maximum WebSocket-send duration, 18 producer-starvation events, and 75 underflows/rebuffers totaling approximately 2,505 ms. Both services were rolled back to `c47a53352d8cfc57b8de62d9cba4ef9e245820be` before Task 16.1.4 edits. That safety rollback is the only Task 16.1.4 production mutation; do not deploy the new fix or place another call in this task.

Task 16.1.4 configures `OutboundPacketDuration=20 ms`, `OutboundStartupBufferDuration=280 ms`, `OutboundLowWaterDuration=140 ms`, `OutboundHighWaterDuration=280 ms`, and `OutboundMaximumSendAheadDuration=300 ms`. Scheduler and send delays compound on the same reserve timeline: the conservative measured budget is the normal 20 ms interval plus 161.257 ms wake lateness plus 76.115 ms awaiting the WebSocket send, or 257.372 ms. The 280 ms high-water target leaves 22.628 ms measured margin. Crossing low water enters one proactive-refill state and real available media is sent toward high water without waiting for zero. A true exhaustion starts one rebuffer episode; a completed short tail resumes with its available real media rather than waiting indefinitely. The pre-send 300 ms check remains authoritative.

Remote reserve is based only on successfully completed Twilio media sends and their represented duration minus elapsed monotonic playback time. It is recomputed after delayed sends. Local queues, unsent packets, canceled/cleared media, and prior generations do not count. `clear` resets reserve/refill/rebuffer state. Response telemetry includes startup/low/high/maximum thresholds, proactive-refill count, average send duration, and the scheduler/send/starvation/underflow/rebuffer/minimum/maximum fields. Keep it response-level and numeric; never log packet payloads, audio, Base64, transcript or response text, prompts, phone numbers, patient information, secrets, or arbitrary provider bodies.

The deterministic startup increase is exactly 180 ms over the failed candidate's 100 ms reserve. Long streaming responses still send first media before TTS completion; short final responses flush below threshold without silence. This is a local validation result only. Do not claim production jitter is fixed until both services are later deployed from the exact reviewed Task 16.1.4 SHA and one separately authorized Task 16.2 revalidation call passes.
...
### Task 16.4 adaptive-language candidate (local validation only)

Task 16.4 starts from production-validated and promoted SHA `f0fc221ea0e2e98e742f313e6cda98c33f854528`. It adds a tenant/location-scoped default call language, an optional bounded outbound override, and in-call explicit/automatic switching without changing the validated Twilio transport controller. The supported startup values are `en-US`, `es-US`, and `es-PR`; existing location, call, and conversation rows migrate safely to English-compatible non-null state. Run the dedicated migration host locally before web integration tests. Do not apply these migrations directly to production, deploy Render, synchronize the Blueprint, change production environment values, or place a call during Task 16.4.

The runtime order is explicit caller request, startup override, confirmed automatic switch, location default, then English fallback. Automatic switching defaults to two consecutive meaningful alternate-language turns (at least four words and 16 characters, and confidence at least `0.75` when supplied). The two-turn cooldown and three-turn reversal requirement prevent flapping. Mixed-language/Spanglish turns, short acknowledgements, low-confidence evidence, and name/address fragments retain the current language. Every accepted switch updates the same durable Conversation and keeps STT, agent instructions, TTS, greeting/fallback language, metrics, and safe diagnostics aligned.

`gpt-transcribe` is the committed OpenAI transcription default because its completed-audio streaming result can return detected-language codes with the final transcript in the same provider request. Do not add a second paid language-detection call. Detection metadata and accuracy remain provider-dependent; explicit requests remain deterministic when automatic metadata is absent. `OpenAI__ApiKey` remains a secret Render value and must never be copied into source, tests, logs, screenshots, or local handoff output.

After the exact Task 16.4 candidate passes local review, Task 16.5 is the separate production-validation action. Deploy only that exact SHA, require healthy BFF and worker readiness, and perform controlled calls for English start, Spanish start, both explicit directions, repeated-evidence automatic switching, Spanglish stability, and barge-in during a switch. Reconfirm streaming, all response marks, and zero playback underflow/rebuffer/producer-starvation regressions before making any production multilingual claim. Do not begin scheduling work as part of either task.

### Task 16.4.1 outbound-creation correction (local validation only)

The first Task 16.5 attempt stopped before any call was created. Its PowerShell harness consumed the single `/bff/v1/calls` response as an array object and copied the projected `toNumber` collection into `destinationNumber`. The resulting JSON contained a 20-element array where `OutboundTransportRequest` requires one string. ASP.NET failed while binding the request body, before the endpoint, worker-readiness probe, language resolver, EF transaction, outbox, durable telephony operation, or provider adapter ran. The supported scalar request had not failed. The generic exception handler classified that body-binding exception as `unexpected_error`, which made the boundary appear to be a server-side call-creation failure.

Task 16.4.1 keeps the scalar API contract and maps malformed JSON/body binding to bounded `invalid_request`. A request-scoped call-creation diagnostic records only bounded stages and internal state: `model_binding`, `request_accepted`, `authorization_passed`, `worker_ready`, `request_validated`, `language_validated`, `language_resolved`, `idempotency_checked`, `call_aggregate_created`, `dispatch_created`, `outbox_created`, `persistence_started`, `persistence_completed`, `transaction_committed`, and `response_returned`. Successful completion and failure records may include trace/correlation ID, fixed operation and method, bounded role, internal tenant/location/call IDs, exception type, safe category, and persistence booleans. They never include the request path or body, destination, idempotency key, cookies, tokens, credentials, transcript, prompt, audio, patient data, database values, or arbitrary exception messages.

For a future outbound 500, correlate the client trace/correlation identifiers with event `221` and inspect `Stage`, `TransactionBegan`, `CallPersisted`, `OutboxPersisted`, and `DispatchPersisted`. A `model_binding` rejection is client-shape failure and must create no durable work. A failure after `persistence_started` requires supported read-model checks before any retry. Do not place a real call until a correctly typed scalar request passes the migrated PostgreSQL production-registration test with a deterministic provider, creates exactly one call, operation, and initial outbox record, and an idempotent replay creates no duplicate.
