...
### Speech-recognition response boundary

The Task 16.5 controlled matrix produced one isolated first-call `speech_recognition_response_invalid`; its retained production evidence did not include a safe response-shape category. The former adapter assigned that same code to an empty final transcript, malformed JSON, missing or wrongly typed `text`, a missing final SSE event, and an oversized transcript. Consequently, the exact provider response from that call cannot be reconstructed and must not be reported as known. The second controlled call is a separate passing result: it completed its English-to-Spanish conversation and does not count as a failed call.

The adapter now distinguishes bounded result and response-shape categories. A valid final transcript may omit language metadata; a bounded but unsupported detected language remains non-fatal and cannot switch the active language. A successful final response whose `text` is empty or whitespace is `empty_result`: the turn is discarded before caller-turn persistence, language evidence, model generation, or fallback speech, and the session remains available for the next utterance. Malformed JSON, an unexpected content type, missing/wrongly typed `text`, a missing or duplicate final SSE event, an oversized response/transcript, or an HTTP-success provider error envelope remains a bounded provider/schema failure and ends only that turn under the existing failure policy. No extra provider retry was introduced; the configured one-attempt default remains authoritative.

Cancellation wins over a late STT completion. After hangup, caller disconnect, barge-in cancellation, session shutdown, or timeout cancels the active operation, a subsequently returned result is observed only for bounded diagnostics and is rejected before transcript persistence, language evaluation, model generation, or TTS. The single-reader finalized-turn set continues to reject duplicate turn IDs, and duplicate final SSE completion events are invalid rather than producing two caller turns.

The speech-recognition boundary event contains only internal call/conversation/correlation/trace/turn identity and bounded adapter, operation, stage, HTTP-status, content-type, response-shape, language-support, cancellation, retry, and recovery categories. It records transcript/language presence booleans, never transcript text or language values. It must not contain raw/provider payloads or error messages, audio, destination or other phone numbers, request bodies, idempotency keys, prompts, cookies/tokens, API keys, credentials, patient data, or arbitrary database values.
